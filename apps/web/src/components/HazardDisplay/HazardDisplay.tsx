    ...
    <div className="space-y-3">
        ...
        <div>
          <div
            className={`text-2xl font-bold flex items-center justify-center gap-1 ${
              hazard.anySevere ? "text-red-400" : getScoreColorClass(hazard.worstHazard)
            }`}
          >
      ...
      {hazard.anySevere && (
        <div className="flex items-center gap-2 rounded-md border border-red-500/40 bg-red-500/10 px-3 py-2">
          <TriangleAlert className="w-4 h-4 text-red-400 flex-shrink-0" aria-hidden="true" />
        ...
        {hazard.hazards.map((h) => (
          <li key={h.hazardType}>
            <CollapsibleCard
              ...
              header={<span className="font-medium text-slate-100 capitalize">{h.hazardType}</span>}
              headerEnd={
            ...
            >
              <dl className="text-xs text-slate-400 space-y-1">
                <div className="flex justify-between">
                  ...
                  <dd className="text-slate-300">{h.source}</dd>
                </div>
                <div className="flex justify-between">
                  <dt>{t("hazard.dataAsOf", { defaultValue: "Data as of" })}</dt>
                  <dd className="text-slate-300">{h.currencyDate}</dd>
                </div>
                {h.isProxy && (
      ...
      <p className="text-xs text-amber-400 flex items-start gap-2 pt-2 border-t border-slate-700/30">
