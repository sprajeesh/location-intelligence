...
| Walking/cycling routes suddenly 502ing (driving still fine) | `osrm-foot`/`osrm-bike` circuit breaker open, or that instance down/missing its dataset | `docker compose logs osrm-foot` / `osrm-bike`; confirm `./osrm-data/new-zealand-latest-foot.osrm` / `-bicycle.osrm` exist — if not, run `./scripts/setup-osrm.sh` **manually over SSH** (never as part of automated `deploy-api`; it's memory-heavy enough to OOM-kill itself alongside the live stack). `deploy-api` now health-checks all three OSRM ports post-deploy (`.github/workflows/push.yml`), so a deploy fails loudly if a dataset is missing rather than shipping a silent 502 — it does not, and should not, generate the dataset itself |
