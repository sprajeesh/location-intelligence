...
| Walking/cycling routes suddenly 502ing (driving still fine) | `osrm-foot`/`osrm-bike` circuit breaker open, or that instance down/missing its dataset | `docker compose logs osrm-foot` / `osrm-bike`; confirm `./osrm-data/new-zealand-latest-foot.osrm` / `-bicycle.osrm` exist (`./scripts/setup-osrm.sh` if not). In production this dataset check and an OSRM health poll are now part of `deploy-api` (`.github/workflows/push.yml`), so a missing dataset should fail the deploy loudly instead of shipping a silent 502 — if it's happening anyway, check the deploy job's log for why `setup-osrm.sh` or the OSRM health check didn't catch it |
