    ...
    <div className="space-y-3">
      {Array.from({ length: count }).map((_, idx) => (
        <div key={idx} className="space-y-2">
          ...
          <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-100 animate-pulse">
          ...
          {Array.from({ length: 2 }).map((_, itemIdx) => (
            <div
              key={itemIdx}
              className="px-3 py-2 rounded-lg bg-slate-100 animate-pulse"
            >
      ...
      <div className="pt-3 border-t border-slate-200 space-y-2">
        ...
        <div className="space-y-2">
          {Array.from({ length: 2 }).map((_, idx) => (
            <div key={idx} className="h-3 w-full bg-slate-200 rounded animate-pulse" />
          ))}
        </div>
      </div>
    </div>
