...
variable "api_ingress_cidr" {
  description = "CIDR allowed to reach the FastAPI backend on port 8000 (plain HTTP, no TLS). Defaults to 0.0.0.0/0 because the only caller -- the Cloudflare Worker running apps/web -- has no fixed egress IP to allowlist; narrow this only if the API sits behind something with a stable source IP."
  type        = string
  default     = "0.0.0.0/0"
}

variable "db_user" {
  description = "Postgres role name for the app database. Not treated as a secret (usernames aren't sensitive); the password is generated by Terraform and stored in OCI Vault -- see vault.tf."
  type        = string
  default     = "gisuser"
}
